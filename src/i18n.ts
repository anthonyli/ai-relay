...
const translations = {
  en: {
    ...
    "import.codexAppProjects": "Codex App projects synced: {count}",
    "import.codexAppRestart": "Quit the Codex App before importing, then reopen it after the restore so its in-memory state does not overwrite the restored project list.",
    "import.codexAppSqliteWarning": "Codex App session files were restored, but its optional thread index was not synced: {reason}",
    ...
    "sync.confirmOverwrite": "Sync will merge the remote backup locally and overwrite the same remote object. Continue?",
    "sync.nonInteractiveYes": "Non-interactive sync requires --yes before overwriting the remote backup.",
  ...
  zh: {
    ...
    "import.codexAppProjects": "已同步 Codex App 项目：{count} 个",
    "import.codexAppRestart": "导入前请完全退出 Codex App；恢复完成后再重新打开，避免 App 内存中的旧状态覆盖已恢复的项目列表。",
    "import.codexAppSqliteWarning": "Codex App 会话文件已恢复，但可选的会话索引未同步：{reason}",
    ...
    "sync.confirmOverwrite": "同步会先在本机合并远程备份，再覆盖同一个远程对象。是否继续？",
    "sync.nonInteractiveYes": "非交互同步覆盖远程备份前必须传入 --yes。",
